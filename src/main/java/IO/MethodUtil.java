...
public class MethodUtil {
    ...
    public static void main(String[] args) throws Exception {
        Map<String, String> map = Files.readAllLines(Paths.get(getClassPath()))
                .stream().map(v -> {
                    v = v.replace("\\s*", "");
                    return v;
                }).collect(Collectors.toMap(v -> v.split("=")[0],
                        v -> v.split("=")[1]));
        ...
        List<String> test = m.retriveSource(map.get(tespStart)).stream()
                .filter(v -> v.startsWith(tespStart)).map(v -> {
                    return firstLetterLowercase(
                            v.substring(tespStart.length()));
                }).collect(Collectors.toList());
        impl.removeAll(test);
    ...
    private List<String> retriveSource(String path)
    ...
    @Test
    public void getPrint() throws URISyntaxException, IOException {
        String v = "testGetCuiPartyListByRole";
        System.out.println(v.substring("test".length()));
    }
    ...
    private static URI getClassPath() throws URISyntaxException {
        return MethodUtil.class.getClassLoader().getResource("test.txt")
                .toURI();
    }
